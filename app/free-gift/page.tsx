...
export default function FreeGiftPage() {
  return (
    <section className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <h1 className="mt-3 font-serif text-4xl font-bold leading-tight tracking-normal text-ink sm:text-5xl">
        AIツール選定チェックリスト
      </h1>
      ...
      <div className="mt-8 grid gap-4">
        {checklist.map((item, index) => (
          <article key={item} className="rounded-lg border border-line bg-surface p-5 shadow-soft">
            <p className="text-sm font-bold text-coral">確認 {index + 1}</p>
            <p className="mt-2 text-lg font-bold leading-8 text-ink">{item}</p>
          </article>
        ))}
      </div>
      <section className="mt-8 rounded-lg border border-line bg-surface p-6 text-ink shadow-soft">
        <h2 className="font-serif text-2xl font-bold tracking-normal">次に読むページ</h2>
